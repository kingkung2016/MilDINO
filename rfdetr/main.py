...
class Model:
    def __init__(self, **kwargs):
        ...
        # model_for_flops = deepcopy(self.model).cpu().eval()
        # flops, params = profile(model_for_flops, inputs=(dummy_input,), verbose=False)
        # print(f"Model Params: {params / 1e6:.2f}M")
        # print(f"Model FLOPs: {flops / 1e9:.2f}G (for input size {self.resolution}x{self.resolution})")
        # del model_for_flops
        # ===================================


        if args.pretrain_weights is not None:
            print('pretrain_weights=', args.pretrain_weights)
        ...
        # 打印预训练权重中所有的键及其参数形状
        # print("Keys and shapes in pretrain checkpoint:")
        ...
        if not args.eval:
            ...
            exclude_keys = []
            # 遍历检查点中的所有键
            for key in list(checkpoint['model'].keys()):  # 使用list()避免在迭代时修改字典
                # 如果键包含"projector"，则添加到排除列表
                if "backbone.0.projector" in key:
                    exclude_keys.append(key)
                    # 直接从检查点字典中移除这些键
                    checkpoint['model'].pop(key)
                    #print(f"Excluded projector key: {key}")
                ...
                elif "class_embed" in key:       #去掉类别嵌入头
                    exclude_keys.append(key)
                    checkpoint['model'].pop(key)
                    # print(f"Excluded class_embed key: {key}")
    ...
    def train(self, callbacks: DefaultDict[str, List[Callable]], **kwargs):
        ...
        if args.eval:
            test_stats, coco_evaluator = evaluate(
                ...
                #model, criterion, postprocessors, data_loader_test, base_ds_test, device, args)   #使用测试集进行测试

            if args.output_dir:
                utils.save_on_master(coco_evaluator.coco_eval["bbox"].eval, output_dir / "eval.pth")

            # 将测试结果加入到result里面
            print(f"Test results: {test_stats['results_json']['class_map']}")
...
#                    checkpoint_paths.append(output_dir / f'checkpoint{epoch:04}.pth')
            ...
            map_regular = test_stats["coco_eval_bbox"][1]    # mAP@0.5
            _isbest = best_map_holder.update(map_regular, epoch, is_ema=False)
            if _isbest:
                best_map_5095 = max(best_map_5095, test_stats["coco_eval_bbox"][0])  
                best_map_50 = max(best_map_50, map_regular)      #选择mAP50作为模型选择依据
                ...
                best_test_stats = test_stats.copy()      # 更新最佳 test_stats
            ...
            if args.use_ema:
                ...
                map_ema = ema_test_stats["coco_eval_bbox"][1]    # mAP@0.5
                ...
                if _isbest:
                    ...
                    best_test_stats = ema_test_stats.copy()      # 更新最佳ema的 test_stats
        ...
        best_is_ema = best_map_ema_50 > best_map_50
        ...
        if utils.is_main_process():
            ...
            results = best_test_stats["results_json"]     # 使用最佳 test_stats 的结果
    ...
    def export(self, output_dir="output", infer_dir=None, simplify=False, backbone_only=False, opset_version=17,
        ...
        if shape is None:
            ...
            if shape[0] % 16 != 0 or shape[1] % 16 != 0:
                raise ValueError("Shape must be divisible by 16")
...
def populate_args(
        ...
        # Model parameters
        pretrained_encoder=None,        #主干网络权重
        pretrain_weights=None,          #DETR权重
        pretrain_exclude_keys=None,
        pretrained_distiller=None,

        # Backbone parameters
        encoder=None,         #主干网络名称
        vit_encoder_num_layers=12,      # 考虑修改问题
        window_block_indexes=None,
        position_embedding='sine',      #位置编码方式
        out_feature_indexes=[-1],
        freeze_encoder=False,           # 冻结主干
        layer_norm=False,
        rms_norm=False,
        backbone_lora=False,
        ...
        export_onnx=True,
        onnx_simplify=True,
        onnx_output_dir="onnx",
        onnx_opset_version=17,
        onnx_backbone_only=False,
        ...
        warmup_epochs=1,
        lr_scheduler='step',        #学习率调整方式
        lr_min_factor=0.1,            # cosine衰减到最低值
    ...
    args = argparse.Namespace(
        ...
        onnx_backbone_only=onnx_backbone_only,
        **extra_kwargs
    )
    return args