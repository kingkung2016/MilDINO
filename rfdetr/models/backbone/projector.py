...
from rfdetr.models.backbone.CCFM_max import AdvancedRepC3
from rfdetr.models.backbone.ACmix import C2PSA_ACmix
    ...
    def __init__(
        ...
        for scale in scale_factors:
            ...
            layers = [
                ...
                #AdvancedRepC3(in_dim, out_channels, num_blocks, e=1.0, norm_type='ln', e_dynamic=True, attention=True, multi_scale=True),
...
if __name__ == "__main__":
    ...
    # 创建 C2F 模块实例
    # 参数: c1=64 (输入通道), c2=128 (输出通道), n=3 (Bottleneck层数), e=0.5 (扩展比例)
    c2f_module = C2f(c1=input_channels, c2=128, n=3, e=0.5, act='silu', layer_norm=True)

    # 前向传播
    output = c2f_module(input_tensor)
    print(f"C2F 输出张量形状: {output.shape}")
    ...
    # 计算参数量
    total_params = sum(p.numel() for p in c2f_module.parameters())
    print(f"\n总参数量: {total_params:,}")
