...
def export_onnx(output_dir, model, input_names, input_tensors, output_names, dynamic_axes, backbone_only=False, verbose=True, opset_version=17):
    ...
    # 尝试将权重嵌入到ONNX文件中，避免外部数据文件
    # 后处理：合并外部数据，避免一堆权重文件
    # try:
    #     model = onnx.load(output_file)
    #     # 将所有权重合并到单个外部文件，避免产生大量独立文件
    #     weight_file = output_file + ".weights"
    #     onnx.save_model(
    #         model,
    #         output_file,
    #         save_as_external_data=True,
    #         all_tensors_to_one_file=True,
    #         location=os.path.basename(weight_file),
    #         size_threshold=1024,
    #         convert_attribute=False
    #     )
    #     print(f'External weights consolidated into {weight_file}')
    # except Exception as e:
    #     print(f'Warning: failed to post-process ONNX file: {e}')
